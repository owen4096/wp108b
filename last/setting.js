...
<table>
  ...
  <tbody>
    ...
    <tr><td>溫度</td><td><textarea id="iceorhot"></textarea></td></tr>
    <tr><td>甜度</td><td><textarea id="suger"></textarea></td></tr>
  </tbody>
</table>
<br/>
...
Setting.start = function () {
  ...
  Ui.id('iceorhot').value = JSON.stringify(Shop.iceorhot, null, 2)
  Ui.id('suger').value = JSON.stringify(Shop.suger, null, 2)
}
...
Setting.save = function () {
  try {
    ...
    Shop.iceorhot = JSON.parse(Ui.id('iceorhot').value)
    Shop.suger = JSON.parse(Ui.id('suger').value)
  } catch (error) {
  ...
  localStorage.setItem('Shop.iceorhot', Ui.id('iceorhot').value)
  localStorage.setItem('Shop.suger', Ui.id('suger').value)
